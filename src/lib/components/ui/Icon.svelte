<script module lang="ts">
	/**
	 * Minimal monochrome stroke icons (paths from Lucide, ISC licence).
	 * They draw with `currentColor`, so they follow the surrounding text colour.
	 */
	const ICONS = {
...
<script lang="ts">
	...
	interface Props {
		name: IconName;
		/** Extra classes; size defaults to 1em so the icon scales with the text. */
		class?: ClassValue;
		strokeWidth?: number;
	}
